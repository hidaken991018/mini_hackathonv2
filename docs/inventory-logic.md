...
| 期限推定 | `lib/expiry-defaults.ts` | 食材名→デフォルト賞味/消費期限 |
...
### 5.2 expireDate と consumeBy の違い

| 項目 | expireDate（賞味期限） | consumeBy（消費期限） |
|------|----------------------|---------------------|
| 意味 | 美味しく食べられる期限 | 安全に食べられる期限 |
| 期限後 | 過ぎても食べられる場合がある | 過ぎたら食べない方が良い |
| 日数 | 短い（`defaultExpireDays`） | 長い（`defaultConsumeDays`） |

### 5.3 食材カテゴリ別デフォルト期限

`lib/expiry-defaults.ts` で定義。上から順に検索し、最初にマッチしたカテゴリを使用する。
具体的なキーワードが先、汎用的なキーワードが後に配置されている。

| カテゴリ | 賞味期限 (日) | 消費期限 (日) | 代表的なキーワード |
|---------|:----------:|:----------:|-----------------|
| 魚介類 | 1 | 2 | 鮭, サーモン, マグロ, 刺身, エビ, イカ, サバ, 魚 |
| 生鮮肉類 | 2 | 3 | 鶏, 豚, 牛, ひき肉, ハム, ベーコン, ソーセージ, 肉 |
| 葉物野菜 | 3 | 5 | レタス, ほうれん草, 小松菜, もやし, ニラ, 大葉 |
| パン | 3 | 5 | パン, 食パン, ベーグル, クロワッサン, バゲット |
| 豆腐・納豆 | 5 | 7 | 豆腐, 納豆, 油揚げ, 厚揚げ, こんにゃく |
| 乳製品 | 7 | 10 | 牛乳, ヨーグルト, チーズ, 生クリーム |
| キャベツ・白菜 | 7 | 10 | キャベツ, 白菜, ブロッコリー, セロリ, アスパラ |
| 果物 | 7 | 10 | りんご, バナナ, みかん, いちご, レモン, メロン |
| 根菜類 | 14 | 21 | じゃがいも, 人参, 玉ねぎ, 大根, かぼちゃ, ネギ, にんにく |
| 卵 | 14 | 21 | 卵, たまご, タマゴ |
...
flowchart TD
    ...
    D --> G["購入日 + defaultExpireDays = expireDate"]
    D --> H["購入日 + defaultConsumeDays = consumeBy"]
```
