...
export const analyzeReceiptRequestSchema = z
  .object({
    imageData: z
      ...
      .refine((value) => getBase64SizeInBytes(value) <= MAX_RECEIPT_IMAGE_BYTES, {
...
const inventoryItemInputSchema = z
  .object({
    name: z.string().trim().min(1, 'nameは必須です').max(200, 'nameは200文字以下で指定してください'),
    quantityValue: z
      .number({ message: 'quantityValueは数値で指定してください' })
      .positive('quantityValueは0より大きい値で指定してください')
      .optional(),
    quantityUnit: optionalTrimmedStringSchema
      .refine(
    ...
    isStaple: z.boolean({ message: 'isStapleはbooleanで指定してください' }).optional(),
  })
  .strict();
